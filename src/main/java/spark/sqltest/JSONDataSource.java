...
public class JSONDataSource {
    public static void main(String[] args) {
       ...
       //针对学生基本信息DataFrame,注册临时表,然后查询分数大于80分的学生基本信息
        studentInfo.createOrReplaceTempView("student_info");
        ...
        JavaPairRDD<String, Tuple2<Integer, Integer>> students =
                goodStudentNames.javaRDD()
                        .mapToPair((PairFunction<Row, String, Integer>) row -> new Tuple2<>(row.getString(0),Integer.valueOf(String.valueOf(row.getLong(1)))))
                        .join(goodinfo.javaRDD().mapToPair((PairFunction<Row, String, Integer>) row -> {return new Tuple2<>(row.getString(0), Integer.valueOf(String.valueOf(row.getLong(1))));}));
        ...
        dataFrame.write().option("timestampFormat", "yyyy/MM/dd HH:mm:ss ZZ")
                .format("studentScoresDF")
                .save("C:\\Users\\Administrator\\Desktop\\good.studentScoresDF");
    }
}
